...
# based on https://github.com/Nikorasu/LiveWhisper, used under terms of MIT license
...
class StreamHandler:
    def __init__(self, assist=None, server=None):
    ...
    def process(self):
        if self.fileready:
            ...
            self.server.send_message_to_all(result['text'])
...
def main():
    try:
        server = WebsocketServer(host='127.0.0.1', port=23023, loglevel=logging.INFO)
        server.run_forever(threaded=True)
        handler = StreamHandler(server=server)
        handler.listen()
